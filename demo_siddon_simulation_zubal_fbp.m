...
% scanner.ScannerName = '2DcylinderPET';
% scanner.NrSectorsAxial = 1;          % number of sectors axial - contains modules 
% scanner.NrSectorsTrans = 384;         % number of sectors transaxial - contains modules side by side in the same slope angle
...
% scanner.NrModulesAxial = 1;          % number of modules axial - contains detectors
% scanner.NrModulesTrans = 1;          % number of modules transaxial - contains detectors
% scanner.ModulesGapAxial = 1;       % module gap axial
% scanner.ModulesGapTrans = 1;       % module gap transaxial
% scanner.NrCrystalsAxial = 1;         % number of detectors per module axial
% scanner.NrCrystalsTrans = 1;         % number of detectors per module transaxial
% scanner.CrystalSize = [1.05, 1.05];   % size of detector transaxial and axial
...
disp('--- FBP reconstruction ...')
for i_reocn_frame = 1 : length(recon_frame)
    recon_frame_temp = recon_frame(i_reocn_frame);
    x(:, i_reocn_frame) = fbp2d_simulation(G, imgsiz, voxel_size, prjsiz, yi(:, recon_frame_temp), ni(:, recon_frame_temp), ri(:, recon_frame_temp), fbptype, filter, frequency_scaling, interp_method, use_psf, FWHM);
end
for i_frame = 1 : length(recon_frame)
    figure
    imagesc(reshape(x(:, i_frame), imgsiz))
    axis image; axis off
    title(['FBP Frame : ' num2str(recon_frame(i_frame))])
end