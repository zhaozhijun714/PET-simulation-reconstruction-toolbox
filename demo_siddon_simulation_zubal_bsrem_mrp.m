...
subset_number = 8;
recon_frame = [2, 12, 24];
relaxation_parameter = 8;
use_psf = 1;
FWHM = 0.75;
voxel_size = double(reconstruction.voxel_size(1:2));
disp('--- BSREM MRP reconstruction ...')
for i_reocn_frame = 1 : length(recon_frame)
    recon_frame_temp = recon_frame(i_reocn_frame);
    x(:, i_reocn_frame) = bsrem_simulation_mrp(G, xinit, imgsiz, voxel_size, prjsiz, yi(:, recon_frame_temp), ni(:, recon_frame_temp), ri(:, recon_frame_temp), maxit, subset_number, use_psf, FWHM, relaxation_parameter, beta, medx, medy);
end
for i_frame = 1 : length(recon_frame)
    figure
    imagesc(reshape(x(:, i_frame), imgsiz))
    axis image; axis off
    title(['BSREM MRP Frame : ' num2str(recon_frame(i_frame))])
end