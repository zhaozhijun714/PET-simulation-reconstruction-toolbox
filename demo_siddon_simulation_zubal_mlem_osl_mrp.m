...
disp('--- MLEM OSL MRP reconstruction ...')

for i_reocn_frame = 1 : length(recon_frame)
    recon_frame_temp = recon_frame(i_reocn_frame);
    x(:, i_reocn_frame) = mlem_simulation_osl_mrp(G, xinit, imgsiz, voxel_size, yi(:, recon_frame_temp), ni(:, recon_frame_temp), ri(:, recon_frame_temp), maxit, use_psf, FWHM, beta, medx, medy);
end
...
for i_frame = 1 : length(recon_frame)
    ...
    axis image; axis off
    title(['MLEM OSL MRP Frame : ' num2str(recon_frame(i_frame))])
end
