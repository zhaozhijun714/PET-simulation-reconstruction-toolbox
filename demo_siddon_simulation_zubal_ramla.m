...
maxit = 60;
recon_frame = [2, 12, 24];
relaxation_parameter = 3;
use_psf = 1;
FWHM = 0.75;
voxel_size = double(reconstruction.voxel_size(1:2));
disp('--- RAMLA reconstruction ...')
for i_reocn_frame = 1 : length(recon_frame)
    recon_frame_temp = recon_frame(i_reocn_frame);
    x(:, i_reocn_frame) = ramla_simulation(G, xinit, imgsiz, voxel_size, yi(:, recon_frame_temp), ni(:, recon_frame_temp), ri(:, recon_frame_temp), maxit, use_psf, FWHM, relaxation_parameter);
end
for i_frame = 1 : length(recon_frame)
    figure
    imagesc(reshape(x(:, i_frame), imgsiz))
    axis image; axis off
    title(['RAMLA Frame : ' num2str(recon_frame(i_frame))])
end