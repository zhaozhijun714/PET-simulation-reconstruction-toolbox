...
for i_reocn_frame = 1 : length(recon_frame)
    ...
    x(:, i_reocn_frame) = mlem_simulation_gradient_descent(G, xinit, imgsiz, voxel_size, yi(:, recon_frame_temp), ni(:, recon_frame_temp), ri(:, recon_frame_temp), maxit, alpha, use_psf, FWHM);
end
for i_frame = 1 : length(recon_frame)
    figure
    imagesc(reshape(x(:, i_frame), imgsiz))
    axis image; axis off
    title(['MLEM Frame : ' num2str(recon_frame(i_frame))])
end