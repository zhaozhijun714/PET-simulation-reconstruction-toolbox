...
K = buildSparseK_simulation(N, W);
toc;    
disp('--- KEM reconstruction ...')
for i_reocn_frame = 1 : length(recon_frame)
    recon_frame_temp = recon_frame(i_reocn_frame);
    a(:, i_reocn_frame) = kem_simulation(G, xinit, imgsiz, voxel_size, yi(:, recon_frame_temp), ni(:, recon_frame_temp), ri(:, recon_frame_temp), maxit, K, use_psf, FWHM);
    x(:, i_reocn_frame) = K * a(:, i_reocn_frame);
end
for i_frame = 1 : length(recon_frame)
    figure
    imagesc(reshape(x(:, i_frame), imgsiz))
    axis image; axis off
    title(['KEM Frame : ' num2str(recon_frame(i_frame))])
end