function x = mlem_simulation_dem_mrp(G, x0, imgsiz, voxel_size, yi, ni, ri, maxit, use_psf, FWHM, beta, medx, medy)
numpix = prod(imgsiz);
if isempty(x0)
    x0 = ones(numpix,1);
end
...
for it = 1 : maxit     
    ...
    x(~mask) = 0;
    temp1 = wx - beta * grad;
    temp2 = sqrt(temp1 .^ 2 + 4 * beta .* grad .* x .* wx);
    temp2 = real(temp2);
    x = 2 * x .* wx ./ (temp1 + temp2);
    x(isnan(x)) = 0;
    x(isinf(x)) = 0;
    x(x<0) = 0;
end
