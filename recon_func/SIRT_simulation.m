function x = SIRT_simulation(G, x0, imgsiz, voxel_size, yi, ni, ri, maxit, use_psf, FWHM)
numpix = prod(imgsiz);
if isempty(x0)
    x0 = ones(numpix,1);
end
if isempty(maxit)
    maxit = 10;
end
sens = proj_back_simulation(G, ni, imgsiz, voxel_size, use_psf, FWHM);
mask = sens > 0;
x    = max(mean(x0(:)) * 1e-9, x0(:));
x(~mask) = 0;
wx   = sens;
y_norm = proj_forw_simulation(G, ones(size(x)), imgsiz, voxel_size, use_psf, FWHM);
for it = 1 : maxit
    disp(sprintf('iteration %d',it));
    yb = ni .* proj_forw_simulation(G, x, imgsiz, voxel_size, use_psf, FWHM) + ri;
    yy_diff = (yi - yb) ./ y_norm;
    xb = proj_back_simulation(G, ni.*yy_diff, imgsiz, voxel_size, use_psf, FWHM);
    x = x + xb ./ wx;
    x(~mask) = 0;
end